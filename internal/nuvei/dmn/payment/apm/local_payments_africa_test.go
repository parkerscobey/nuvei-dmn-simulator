...
func TestLocalPaymentsAfricaDefaults(t *testing.T) {
	payload, err := LocalPaymentsAfrica(payment.Options{
	...
	if err != nil {
		t.Fatalf("LocalPaymentsAfrica error = %v", err)
	}

	assertField(t, payload, payment.FieldPaymentMethod, LocalPaymentsAfricaPaymentMethod)
	assertField(t, payload, payment.FieldTransactionType, "Sale")
	assertField(t, payload, payment.FieldType, "DEPOSIT")
	assertField(t, payload, payment.FieldCurrency, LocalPaymentsAfricaCurrency)
	assertField(t, payload, payment.FieldTotalAmount, LocalPaymentsAfricaTotalAmount)
...
func TestLocalPaymentsAfricaStatusMappings(t *testing.T) {
	...
	for status, wantPPPStatus := range tests {
		...
		if err != nil {
			t.Fatalf("LocalPaymentsAfrica(%q) error = %v", status, err)
		}
...
func TestLocalPaymentsAfricaDeclinedDefaults(t *testing.T) {
	payload, err := LocalPaymentsAfrica(payment.Options{
		MerchantID:          "merchant-id",
		MerchantSiteID:      "site-id",
		MerchantSecretKey:   "secret",
